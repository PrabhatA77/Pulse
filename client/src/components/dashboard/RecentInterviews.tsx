...
const RecentInterviews = ({ interviews }: RecentInterviewsProps) => {
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl dark:border-zinc-800 dark:bg-zinc-900">
      ...
      {interviews.length === 0 ? (
        <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">No interviews yet — start one above to see it here.</p>
      ) : (
          ...
          {interviews.map((interview) => (
            <div key={interview.id} className="flex items-center justify-between gap-3 py-3">
              ...
              <div className="flex shrink-0 items-center gap-3">
                ...
                {/* Submissions saved via the new split submit/analyze flow may not
                    have feedback yet — nothing to review until they're analyzed. */}
                {interview.feedback ? (
                  <button
