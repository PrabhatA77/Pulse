...
const AdminProblemFormPage = () => {
  ...
  const [parameters, setParameters] = useState<ParamFormState[]>([emptyParam()]);
  const [returnType, setReturnType] = useState<ParamType>("int");
  const [testCases, setTestCases] = useState<TestCaseFormState[]>([emptyTestCase()]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setParameters(data.parameters.length > 0 ? data.parameters : [emptyParam()]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      try {
        ...
        toast.error(`Test case ${i + 1}: input/expected output must be valid JSON`);
        return;
      }
    }
    ...
    const payload: ProblemFormPayload = {
      ...
      parameters: parameters.map((p) => ({ name: p.name.trim(), type: p.type })),
      returnType,
              ...
              {testCases.map((tc, i) => (
                <div key={i} className="rounded-xl border border-zinc-200 p-4 dark:border-zinc-800">
                  ...
                  <div className="flex flex-col gap-3">
                    <div>
                      <label className={labelClass}>Input (JSON object, keyed by parameter name)</label>
                      <textarea
                        className={`${inputClass} min-h-20 resize-y font-mono text-xs`}
                        value={tc.inputText}
                        onChange={(e) => updateTestCase(i, { inputText: e.target.value })}
                        placeholder='{"nums": [2,7,11,15], "target": 9}'
                      />
                    </div>
                    <div>
                      <label className={labelClass}>Expected output (JSON value)</label>
                      <input
                        className={`${inputClass} font-mono text-xs`}
                        value={tc.expectedOutputText}
                        onChange={(e) => updateTestCase(i, { expectedOutputText: e.target.value })}
                        placeholder="[0,1]"
                      />
                    </div>
                    <div>
